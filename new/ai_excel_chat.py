                ...
                if response.status_code == 200:
                    try:
                        ...
                        try:
                            ...
                            in_tokens = usage.get("promptTokenCount", prompt_len // 4)
                            ...
                            log_api_usage(
                                ...
                                prompt_type=getattr(self, '_current_prompt_type', 'custom'),
...
"""
    ...
    def get_context_for_question(self, question: str, model: str = DEFAULT_MODEL,
        ...
        # ── DEBUG: Write token breakdown to file (only when ADF_DEBUG=1) ──
        if os.environ.get("ADF_DEBUG", "0") == "1":
            try:
...
def initialize_ai_session_state():
    ...
    defaults = {
        ...
        "ai_session_cost": 0.0,
        "ai_session_calls": 0,
        "ai_limit_history": True,
        "ai_max_history": 10,
        # Multi-provider
        "ai_active_provider": "gemini",
        "ai_enable_caching": True,  # Enabled by default (Claude, Gemini, OpenAI support)
    }
...
def render_ai_sidebar():
    """Render unified multi-provider AI sidebar with provider selector, model picker, and per-provider chat history."""
    initialize_ai_session_state()

    # ── Provider definitions ──
    PROVIDERS = {
        "gemini":  {"icon": "🟢", "label": "Gemini",  "color": "#34a853", "key_env": "GEMINI_API_KEY"},
        "openai":  {"icon": "🟦", "label": "OpenAI",  "color": "#10a37f", "key_env": "OPENAI_API_KEY"},
        "claude":  {"icon": "🟣", "label": "Claude",  "color": "#7c3aed", "key_env": "ANTHROPIC_API_KEY"},
    }

    # Model registries per provider (March 2026 accurate)
    PROVIDER_MODELS = {
        "gemini": {
            "gemini-2.5-pro": {"name": "Gemini 2.5 Pro", "icon": "🧠", "ctx": "2M", "price": "$1.25/$10", "badge": "Deep analysis"},
            "gemini-2.5-flash": {"name": "Gemini 2.5 Flash", "icon": "⚡", "ctx": "1M", "price": "$0.30/$2.50", "badge": "Fast + code exec"},
            "gemini-2.5-flash-lite": {"name": "Flash Lite", "icon": "💨", "ctx": "1M", "price": "$0.10/$0.40", "badge": "Cheapest"},
        },
        "openai": {
            "gpt-5.4": {"name": "GPT-5.4", "icon": "🧠", "ctx": "1M", "price": "$2.50/$15", "badge": "Best reasoning"},
            "gpt-5.4-mini": {"name": "GPT-5.4 Mini", "icon": "⚡", "ctx": "400K", "price": "$0.75/$4.50", "badge": "Fast + capable"},
            "gpt-5.4-nano": {"name": "GPT-5.4 Nano", "icon": "💨", "ctx": "400K", "price": "$0.20/$1.25", "badge": "Ultra cheap"},
            "gpt-4.1": {"name": "GPT-4.1", "icon": "🔷", "ctx": "1M", "price": "$2/$8", "badge": "Code + data"},
            "gpt-4.1-nano": {"name": "GPT-4.1 Nano", "icon": "💎", "ctx": "1M", "price": "$0.05/$0.20", "badge": "Dirt cheap"},
        },
        "claude": {
            "claude-sonnet-4-6": {"name": "Sonnet 4.6", "icon": "🎵", "ctx": "1M", "price": "$3/$15", "badge": "Best value"},
            "claude-opus-4-6": {"name": "Opus 4.6", "icon": "🎭", "ctx": "1M", "price": "$5/$25", "badge": "Most capable"},
        },
    }
    ...
    current_provider = st.session_state.get("ai_active_provider", "gemini")
    if current_provider not in provider_options:
        current_provider = "gemini"

    provider_labels = [f"{PROVIDERS[p]['icon']} {PROVIDERS[p]['label']}" for p in provider_options]
    ...
    selected_provider = st.radio(
        ...
        index=selected_idx,
        format_func=lambda p: f"{PROVIDERS[p]['icon']} {PROVIDERS[p]['label']}",
        key="sidebar_provider_radio",
        horizontal=True,
    )
    st.session_state.ai_active_provider = selected_provider
    prov = PROVIDERS[selected_provider]

    # Provider accent bar
    st.markdown(f"""<div style="height:3px;background:{prov['color']};border-radius:2px;margin:-5px 0 10px 0;"></div>""", unsafe_allow_html=True)
    ...
    if selected_provider == "gemini":
        ...
        if key_mgr.has_keys:
            ...
            st.success(f"✅ {key_mgr.key_count} Keys  •  `{masked}`")
        else:
            st.warning("⚠️ No Gemini keys")
            user_key = st.text_input("Gemini API Key:", type="password", key="sidebar_gemini_key", help="Get free → ai.google.dev")
            if user_key:
                st.session_state.user_gemini_api_key = user_key
                st.rerun()
    elif selected_provider == "openai":
        import os as _os
        oai_keys = [k for k in [_os.environ.get("OPENAI_API_KEY"), st.session_state.get("user_openai_key")] if k]
        if oai_keys:
            masked = f"{oai_keys[0][:4]}...{oai_keys[0][-4:]}"
            st.success(f"✅ OpenAI  •  `{masked}`")
        else:
            st.warning("⚠️ No OpenAI key")
            user_key = st.text_input("OpenAI API Key:", type="password", key="sidebar_oai_key", help="platform.openai.com")
            if user_key:
                st.session_state.user_openai_key = user_key
                st.rerun()
    elif selected_provider == "claude":
        import os as _os
        claude_keys = [k for k in [_os.environ.get("ANTHROPIC_API_KEY"), st.session_state.get("user_anthropic_key")] if k]
        if claude_keys:
            masked = f"{claude_keys[0][:4]}...{claude_keys[0][-4:]}"
            st.success(f"✅ Claude  •  `{masked}`")
        else:
            st.warning("⚠️ No Anthropic key")
            user_key = st.text_input("Anthropic API Key:", type="password", key="sidebar_claude_key", help="console.anthropic.com")
            if user_key:
                st.session_state.user_anthropic_key = user_key
                st.rerun()
    ...
    prov_model_key = f"ai_model_{selected_provider}"
    if prov_model_key not in st.session_state:
        st.session_state[prov_model_key] = model_keys[0] if model_keys else ""

    current_model = st.session_state[prov_model_key]
    if current_model not in model_keys:
        current_model = model_keys[0]
        st.session_state[prov_model_key] = current_model
    ...
    selected_model = st.selectbox(
        ...
        format_func=lambda m: f"{models[m]['icon']} {models[m]['name']}  •  {models[m]['ctx']}  •  {models[m]['price']}",
        key=f"sidebar_model_{selected_provider}",
    )
    st.session_state[prov_model_key] = selected_model

    # Sync with the module-level model state for each provider
    if selected_provider == "gemini":
    ...
    m_info = models.get(selected_model, {})
    if m_info:
        st.caption(f"🏷️ **{m_info.get('badge', '')}**  •  Context: {m_info.get('ctx', '?')}  •  Price: {m_info.get('price', '?')}/M tokens")

    # ── New Chat Button ──
    st.markdown("---")
    pm = ChatPersistenceManager()

    if st.button(f"➕ New {prov['label']} Chat", width="stretch", type='primary'):
        if selected_provider == "gemini":
            st.session_state.ai_chat_id = f"chat_{int(time.time())}"
            st.session_state.ai_chat_history = []
            st.session_state.ai_api_history = []
            ...
            st.session_state.claude_chat_id = f"claude_{int(time.time())}"
            st.session_state.claude_chat_history = []
            st.session_state.claude_file_id = None
        st.rerun()

    # ── Per-Provider Chat History (Accordions) ──
    for prov_key, prov_info in PROVIDERS.items():
        history_key = {
            "gemini": "ai_chat_history",
            "openai": "gpt_chat_history",
            "claude": "claude_chat_history",
        }.get(prov_key, "")

        hist = st.session_state.get(history_key, [])
        msg_count = len(hist)
        badge = f" ({msg_count})" if msg_count > 0 else ""

        with st.expander(f"{prov_info['icon']} {prov_info['label']} History{badge}", expanded=(prov_key == selected_provider)):
            saved = pm.get_history()
            
            # Filter by provider prefix
            prefix_map = {"gemini": "chat_", "openai": "gpt_", "claude": "claude_"}
            prov_prefix = prefix_map.get(prov_key, "")
            prov_saved = [c for c in saved if c["id"].startswith(prov_prefix)]
            
            current_id_map = {
                "gemini": st.session_state.get("ai_chat_id", ""),
                "openai": st.session_state.get("gpt_chat_id", ""),
                "claude": st.session_state.get("claude_chat_id", "")
            }
            current_chat_id = current_id_map.get(prov_key, "")

            if prov_saved:
                for chat in prov_saved[:8]:
                    chat_id = chat["id"]
                    title = chat["title"]
                    ...
                    try:
                        ...
                        if delta.total_seconds() < 60: time_label = "just now"
                        elif delta.total_seconds() < 3600: time_label = f"{int(delta.total_seconds() / 60)}m"
                        elif delta.total_seconds() < 86400: time_label = f"{int(delta.total_seconds() / 3600)}h"
                        else: time_label = f"{int(delta.days)}d"
                    except Exception:
                        time_label = ""
                            ...
                            if not is_current:
                                ...
                                if data:
                                    if prov_key == "gemini":
                                        ...
                                        st.session_state.ai_api_history = data.get("api_history", [])
                                        st.session_state.ai_total_tokens_used = data.get("total_tokens", 0)
                                        st.session_state.ai_model = data.get("model", st.session_state.get("ai_model", ""))
                                    elif prov_key == "openai":
                                        st.session_state.gpt_chat_id = data["chat_id"]
                                        st.session_state.gpt_chat_history = data.get("chat_history", [])
                                        st.session_state.gpt_api_history = data.get("api_history", [])
                                        st.session_state.gpt_total_tokens_used = data.get("total_tokens", 0)
                                        st.session_state.gpt_model = data.get("model", st.session_state.get("gpt_model", ""))
                                    elif prov_key == "claude":
                                        st.session_state.claude_chat_id = data["chat_id"]
                                        st.session_state.claude_chat_history = data.get("chat_history", [])
                                        st.session_state.claude_api_messages = data.get("api_history", [])
                                        st.session_state.claude_model = data.get("model", st.session_state.get("claude_model", ""))
                                        
                                    st.session_state.ai_active_provider = prov_key
                                    st.rerun()
                    with c2:
                        if st.button("🗑️", key=f"del_{chat_id}", help="Delete"):
                            ...
                            if current_chat_id == chat_id:
                                if prov_key == "gemini":
                                    st.session_state.ai_chat_id = f"chat_{int(time.time())}"
                                    st.session_state.ai_chat_history, st.session_state.ai_api_history = [], []
                                    st.session_state.ai_total_tokens_used = 0
                                elif prov_key == "openai":
                                    st.session_state.gpt_chat_id = f"gpt_{int(time.time())}"
                                    st.session_state.gpt_chat_history, st.session_state.gpt_api_history = [], []
                                    st.session_state.gpt_total_tokens_used = 0
                                elif prov_key == "claude":
                                    st.session_state.claude_chat_id = f"claude_{int(time.time())}"
                                    st.session_state.claude_chat_history, st.session_state.claude_api_messages = [], []
                            st.rerun()
            else:
                st.caption(f"No saved {prov_info['label']} chats")

    # ── Session Cost Summary ──
    st.markdown("---")
    gemini_cost = st.session_state.get("ai_session_cost", 0.0)
    gpt_cost = st.session_state.get("gpt_session_cost", 0.0)
    claude_cost = st.session_state.get("claude_session_cost", 0.0)
    total_cost = gemini_cost + gpt_cost + claude_cost

    if total_cost > 0:
        st.markdown(f"""
<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px;font-size:0.82rem;">
<div style="font-weight:700;margin-bottom:6px;">💰 Session Costs</div>
<div style="display:flex;justify-content:space-between;"><span>🟢 Gemini</span><span>${gemini_cost:.4f}</span></div>
<div style="display:flex;justify-content:space-between;"><span>🟦 OpenAI</span><span>${gpt_cost:.4f}</span></div>
<div style="display:flex;justify-content:space-between;"><span>🟣 Claude</span><span>${claude_cost:.4f}</span></div>
<div style="border-top:1px solid rgba(255,255,255,0.15);margin-top:6px;padding-top:6px;display:flex;justify-content:space-between;font-weight:700;"><span>Total</span><span>${total_cost:.4f}</span></div>
</div>
""", unsafe_allow_html=True)

    # ── Advanced Settings (Caching) ──
    st.markdown("---")
    st.session_state.ai_enable_caching = st.checkbox(
        "⚡ Enable Prompt Caching",
        value=st.session_state.get("ai_enable_caching", True),
        help="Caches system prompt and file contents to save up to 90% on input token costs.",
    )
    if selected_provider == "claude":
        st.session_state.claude_enable_thinking = st.checkbox(
            "💭 Enable Extended Thinking",
            value=st.session_state.get("claude_enable_thinking", False),
            help="Claude will stream its internal reasoning process before answering. Higher quality, but slower.",
        )
    ...
    if all_pipeline_names:
        with st.expander(f"🔍 **Smart Pipeline Filter** — {len(all_pipeline_names)} pipelines available ({filter_label})", expanded=False):
                ...
                if pipeline_df is not None and "Folder" in pipeline_df.columns:
                    ...
                    for fi, folder in enumerate(folders):
                        ...
                        with col:
                            ...
                            if st.button(f"📁 {folder} ({count})", key=f"folder_{fi}", width="stretch"):
                ...
                if pipeline_df is not None and "Complexity" in pipeline_df.columns:
                    ...
                    for ci, level in enumerate(complexity_levels):
                        ...
                        with col:
                            ...
                            if st.button(f"{icon} {level} ({count})", key=f"complexity_{ci}", width="stretch"):
                    ...
                    if "ImpactLevel" in pipeline_df.columns:
                        ...
                        for ii, impact in enumerate(impact_levels):
                            ...
                            with col:
                                ...
                                if st.button(f"{icon} {impact} ({count})", key=f"impact_{ii}", width="stretch"):
            ...
            with filter_tab3:
                ...
                with mcol1:
                    if st.button("📋 Select All", key="filter_select_all", width="stretch"):
                        ...
                        st.rerun()
                with mcol2:
                    if st.button("🧹 Clear All", key="filter_clear_all", width="stretch"):
    ...
    with st.expander("💡 **Quick Questions** — click any to ask the AI", expanded=no_history):
        ...
        for i, (label, question) in enumerate(PRESET_QUESTIONS):
            ...
            with col:
                ...
                if st.button(label, key=f"preset_{i}", width="stretch", help=tooltip):
                    ...
                    st.session_state._ai_prompt_type = "preset"
                    st.rerun()
        ...
        # Filter Usage Guide
        st.markdown("---")
        with st.expander("📖 **Filter Guide: Which filter for which question?**", expanded=False):
            st.markdown("Gemini can process your whole factory at once (up to 2 million tokens), but applying these filters forces its attention on specific areas to guarantee accuracy:")
            guide_data = []
    ...
    with st.expander("✏️ **Custom Question Editor** — write, refine & save your own prompts", expanded=False):
        ...
        for pi, (plabel, pquestion) in enumerate(PRESET_QUESTIONS[:8]):
            with preset_cols[pi % 4]:
                if st.button(plabel.split(" ", 1)[0], key=f"copy_preset_{pi}", width="stretch", help=f"Load '{plabel}' into editor"):
        ...
        with btn_col1:
            if st.button("🚀 **Send**", key="send_custom", width="stretch", type="primary", disabled=not custom_text.strip()):
        ...
        with btn_col2:
            ...
            if st.button("✨ **Refine with AI**", key="refine_custom", width="stretch", disabled=refine_disabled):
...
Return ONLY the final refined prompt text. Do NOT wrap it in markdown code blocks (```). Do NOT include any preamble like 'Here is your prompt:'. Just output the raw prompt text."""
        ...
        with btn_col3:
            # Save current prompt
            if st.button("💾 Save", key="save_custom", width="stretch", disabled=not custom_text.strip()):
        ...
        with btn_col4:
            if st.button("🗑️ Clear", key="clear_custom", width="stretch"):
        ...
        if saved_prompts:
            ...
            for si, sp in enumerate(saved_prompts):
                ...
                with scol2:
                    if st.button("Load", key=f"load_saved_{si}", width="stretch"):
                        st.session_state.custom_question_text = sp["text"]
                        st.rerun()
                with scol3:
                    if st.button("🗑️", key=f"del_saved_{si}", width="stretch"):
                        st.session_state.ai_saved_prompts.pop(si)
                        st.rerun()

    # ── Model Selection Guide ──
    with st.expander("🧠 **Model Guide: How to use Gemini effectively?**", expanded=False):
        st.markdown("""
        To take advantage of Google's massive 2-Million Token context window, strictly follow this guide:

        | 🎯 Your Goal | 🏆 Recommended Model | ⏱️ Why? |
        |---|---|---|
        | **Table Lineage across ALL Pipelines** | **Gemini 3 Flash** | **Required for Free Users!** The massive TPM limit can ingest the entire factory without failing. |
        | **General Counts / Factory Overview** | **Gemini 2.5 Flash** | Blazing fast. Stable and perfect for summarizing `Statistics` and `PipelineAnalysis`. |
        | **Deep Column Lineage / Reasoning** | **Gemini 3.1 Pro** | Google's premier 2026 reasoning model. Unmatched logic for tracing intricate data flows. |
        | **Legacy Workloads** | **Gemini 2.5 Pro** | Highly reliable backup model for massive 2M context dumps. |

        💡 **Pro-Tip on Implicit Caching (Save 80% Costs):** 
        Unlike OpenAI which requires strict filtering, Gemini thrives on **"Brute Force Context"**. If you select your entire factory without filters, Gemini will read it all at once. Because the system prompts are contiguous, Google automatically applies **Implicit Context Caching**, dropping the price of your follow-up questions by up to 80% dynamically!
        
        🛑 **CRITICAL WARNING FOR FREE TIER USERS:** 
        If you use **Gemini Pro** for massive queries, you might hit an HTTP 429 because the Free Tier is heavily rate-limited. You **MUST** use **Gemini Flash** for giant, unfiltered queries unless you have a paid API key!
        """)
    ...
    if msg_count > 0:
        ...
        with ctrl_col1:
            ...
            """, unsafe_allow_html=True)
        with ctrl_col2:
            if st.button("➕ New Chat", key="main_new_chat", width="stretch", type="primary"):
                st.session_state.ai_chat_id = f"chat_{int(time.time())}"
                st.session_state.ai_chat_history = []
                st.session_state.ai_api_history = []
                st.session_state.ai_total_tokens_used = 0
            ...
            with st.chat_message(msg["role"], avatar=icon):
                ...
                if msg["role"] == "assistant":
                    st.download_button(
                        ...
                        data=msg["content"],
                        file_name=f"gemini_response_{i}.md",
                        mime="text/markdown",
                        key=f"dl_gemini_msg_{i}"
                    )
    ...
    if active_question:
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            # 4. Per-message cost display
            try:
                from api_cost_engine import GEMINI_PRICING
                m_cfg = GEMINI_PRICING.get(model)
                if m_cfg:
                    in_t = est_tokens
                    out_t = len(response) // 4
                    msg_cost = (in_t / 1_000_000) * m_cfg.input_per_m + (out_t / 1_000_000) * m_cfg.output_per_m
                    st.session_state.ai_session_cost = st.session_state.get("ai_session_cost", 0.0) + msg_cost
                    st.session_state.ai_session_calls = st.session_state.get("ai_session_calls", 0) + 1
                    cost_str = f"${msg_cost:.6f}" if msg_cost < 0.01 else f"${msg_cost:.4f}"
                    session_total = st.session_state.ai_session_cost
...
"""
    ...
    # Conversation memory — trim to configured max turns
    api_history = st.session_state.get("ai_api_history", [])
    if st.session_state.get("ai_limit_history", True):
        max_msgs = st.session_state.get("ai_max_history", 10) * 2  # turns → messages
        if len(api_history) > max_msgs:
            api_history = api_history[-max_msgs:]
    
    model = st.session_state.get("ai_model", DEFAULT_MODEL)
    client = GeminiClient(model=model)
    client._current_question = question
    client._current_prompt_type = st.session_state.get("_ai_prompt_type", "custom")
