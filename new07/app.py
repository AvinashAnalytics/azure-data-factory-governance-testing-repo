...
try:
    from api_cost_engine import render_cost_analysis_tab
    HAS_COST_TAB = True
except ImportError:
    HAS_COST_TAB = False
...
try:
    from claude_excel_chat import render_claude_chat_tab
    HAS_CLAUDE = True
except ImportError:
    HAS_CLAUDE = False
...
class ADF_Dashboard:
    ...
    def _render_sidebar_theme_selector(self):
        ...
        with st.expander("🎯 Custom Accent Colors", expanded=False):
            ...
            if st.button("🔄 Reset to Theme Defaults", key="reset_accent_colors", width="stretch"):
    ...
    def _process_loaded_sheets(self, data: Dict[str, pd.DataFrame]):
        ...
        aliases = {
            ...
            "TableLineage": ["tablelineage", "Table_Lineage", "TableLinage", "Lineage_Table"],
    ...
    def render_main_content_with_tabs(self):
        ...
        if "ai_suite_unlocked" not in st.session_state:
            st.session_state.ai_suite_unlocked = False

        _ai_suite_label = "🤖 AI Suite" if st.session_state.ai_suite_unlocked else "🔒"
        ...
        tab_labels = ["⚙️ Generate Excel", "📊 Upload & Analyze", _ai_suite_label, "📚 Documentation"]
        ...
        with main_tabs[2]:
            ...
            if not st.session_state.ai_suite_unlocked:
                st.markdown("""
                <style>
                .admin-lock-card {
                    background: linear-gradient(135deg,rgba(30,34,80,0.95) 0%,rgba(60,30,80,0.95) 100%);
                ...
                .admin-lock-icon  { font-size: 3.6rem; margin-bottom: 12px; }
                .admin-lock-title { font-size: 1.55rem; font-weight: 700;
                                    color: #fff; margin-bottom: 6px; }
                .admin-lock-sub   { font-size: 0.92rem; color: rgba(255,255,255,0.55);
                                    margin-bottom: 28px; }
                .admin-lock-badge {
                ...
                <div class="admin-lock-card">
                    ...
                    <div class="admin-lock-title">AI Suite — Admin Only</div>
                    <div class="admin-lock-sub">
                        This section requires administrator access.<br>
                        Enter the admin password to unlock AI capabilities.
                    </div>
                    <div class="admin-lock-badge">🛡️ Protected Area</div>
                </div>
                """, unsafe_allow_html=True)

                lock_l, lock_c, lock_r = st.columns([1, 2, 1])
                with lock_c:
                    pwd_input = st.text_input(
                        "🔑 Admin Password",
                        type="password",
                        placeholder="Enter admin password…",
                        key="ai_suite_pwd_input",
                        label_visibility="collapsed",
                    )
                    unlock_btn = st.button(
                        ...
                        type="primary",
                        width="stretch",
                        key="ai_suite_unlock_btn"
                    )
                    if unlock_btn:
                        if pwd_input == _ADMIN_PASSWORD:
                            st.session_state.ai_suite_unlocked = True
                            st.session_state.ai_suite_pwd_input = ""   # clear field
                            st.rerun()
                        else:
                            st.error("❌ Incorrect password. Please try again.")

                    st.markdown("")
                    st.caption("💡 Contact your system administrator for access.")

            # ════════════════════════════════════════════════════════════════
            # UNLOCKED STATE — full AI Suite content
            # ════════════════════════════════════════════════════════════════
            else:
                # ── Lock button (top-right) ───────────────────────────────
                hdr_left, hdr_right = st.columns([6, 1])
                with hdr_left:
                    st.markdown("### 🤖 AI Suite &nbsp; <span style='font-size:0.75rem;color:#34d399;background:rgba(52,211,153,0.12);padding:3px 10px;border-radius:10px;'>🔓 Admin Unlocked</span>", unsafe_allow_html=True)
                with hdr_right:
                    if st.button("🔒 Lock", key="ai_suite_lock_btn", help="Lock AI Suite"):
                        st.session_state.ai_suite_unlocked = False
                        st.rerun()
                ...
                # Build sub-tab labels dynamically based on available modules
                ai_sub_labels = []
                ai_sub_keys = []
                if HAS_AI_CHAT:
                    ai_sub_labels.append("🤖 Gemini")
                    ai_sub_keys.append("gemini")
                if HAS_CHATGPT:
                    ai_sub_labels.append("🧠 ChatGPT")
                    ai_sub_keys.append("chatgpt")
                if HAS_CLAUDE:
                    ai_sub_labels.append("🟣 Claude AI")
                    ai_sub_keys.append("claude")
                if HAS_COST_TAB:
                    ai_sub_labels.append("💰 Cost Analysis")
                    ai_sub_keys.append("cost")

                if not ai_sub_labels:
                    st.warning("⚠️ No AI modules available. Install dependencies:")
                    st.code("pip install requests anthropic python-dotenv", language="bash")
                else:
                    ai_tabs = st.tabs(ai_sub_labels)
                    tab_idx = 0
                    ...
                    if HAS_CLAUDE:
                        with ai_tabs[tab_idx]:
                            render_claude_chat_tab(excel_data=excel_data)
                        tab_idx += 1

                    if HAS_COST_TAB:
                        with ai_tabs[tab_idx]:
                            render_cost_analysis_tab(excel_data=excel_data if excel_data else None)
                        tab_idx += 1
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    def _render_health_gauge(self):
        """Render health score gauge with real sub-scores from Summary sheet."""
        metrics = self._get_cached_metrics()
        health_score = metrics['health']

        # ── Pull rich sub-scores from the Summary sheet ───────────────────
        # The Summary sheet is a report-style sheet; find the rows we need
        # by scanning for known label text in any column.
        ...
        overall_health_excel = None  # e.g. "82/100" from Excel

        if not summary_df.empty:
            for _, row in summary_df.iterrows():
                for cell_val in row.values:
                    cell_str = str(cell_val).strip()
                    # Check sub-score labels
                    for full_label, short in score_labels.items():
                        if full_label in cell_str:
                            # The numeric value is in the next non-NaN cell in the row
                            vals = [v for v in row.values if str(v).strip() not in ('', 'nan', 'NaN')]
                            for v in vals:
                                sv = str(v).strip()
                                if '/' in sv:
                                    try:
                                        sub_scores[short] = int(sv.split('/')[0])
                                        break
                                    except ValueError:
                                        pass
                    # Check overall health: "82/100" on the same row as "OVERALL HEALTH"
                    if 'OVERALL HEALTH' in cell_str:
                        for v in row.values:
                            sv = str(v).strip()
                            if '/' in sv:
                                try:
                                    overall_health_excel = int(sv.split('/')[0])
                                    break
                                except ValueError:
                                    pass

        # ── Determine status label and gauge color ─────────────────────────
        if health_score >= 90:
        ...
        # ── Delta: compare vs "Good" threshold (75) ────────────────────────
        # Using 75 means:
        #   score > 75  →  positive delta (green arrow ↑)
        #   score < 75  →  negative delta (red arrow ↓)
        # This makes the delta meaningful instead of always -10.
        delta_reference = 75
        ...
        fig = go.Figure(go.Indicator(
            ...
            value=health_score,
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': f"{icon} Health Score — {status}", 'font': {'size': 16}},
            delta={
                'reference': delta_reference,
                'increasing': {'color': PremiumColors.SUCCESS},
                'decreasing': {'color': PremiumColors.DANGER},
                'suffix': f" vs Good ({delta_reference})",
            },
            gauge={
        ...
        # ── How it's calculated ────────────────────────────────────────────
        st.caption(
            f"**Formula:** 100 × (1 − Orphaned ÷ Pipelines) = "
            f"100 × (1 − {metrics['orphaned']} ÷ {metrics['pipelines']}) = **{health_score}**  "
            f"│  Delta = score − Good threshold ({delta_reference})"
        )

        # ── Sub-score tiles from Summary sheet (if available) ─────────────
        if sub_scores or overall_health_excel is not None:
            st.markdown("---")
            st.markdown("**📋 Detailed Health Breakdown** *(from Summary sheet)*")

            tile_cols = st.columns(len(sub_scores) + (1 if overall_health_excel is not None else 0))
            col_idx = 0

            if overall_health_excel is not None:
                with tile_cols[col_idx]:
                    diff = overall_health_excel - health_score
                    st.metric(
                        label="📊 Overall (Excel)",
                        value=f"{overall_health_excel}/100",
                        delta=f"{diff:+d} vs computed" if diff != 0 else "Matches computed",
            ...
            for label, score in sub_scores.items():
                with tile_cols[col_idx]:
                    emoji = score_colors.get(label, '⚪')
                    st.metric(
                        label=f"{emoji} {label}",
                        value=f"{score}/100",
                        delta=f"{score - 75:+d} vs Good" if score != 75 else "At threshold",
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    def render_lineage_tab(self):
        """Render data lineage visualization — both DataLineage and TableLineage sheets."""
        st.markdown("### 🔄 Lineage")
        st.markdown("*Track data flow from source to sink*")

        # ── Two sub-tabs ──────────────────────────────────────────────────
        sub_tab1, sub_tab2 = st.tabs(["📊 Data Lineage", "🗂️ Table Lineage"])

        # ──────────────────────────────────────────────────────────────────
        # SUB-TAB 1: Data Lineage (DataLineage sheet)
        # ──────────────────────────────────────────────────────────────────
        with sub_tab1:
            lineage_df = safe_get_dataframe("DataLineage")

            if lineage_df.empty:
                st.info("📊 No Data Lineage data available")
            else:
                # Metrics row
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                ...
                with col2:
                    st.metric("Sources", lineage_df['Source'].nunique() if 'Source' in lineage_df.columns else 0)
                with col3:
                    st.metric("Sinks", lineage_df['Sink'].nunique() if 'Sink' in lineage_df.columns else 0)
                with col4:
                    st.metric("Pipelines", lineage_df['Pipeline'].nunique() if 'Pipeline' in lineage_df.columns else 0)
                ...
                with col1:
                    ...
                    if 'Pipeline' in lineage_df.columns:
                        pipelines = ["All"] + sorted(lineage_df['Pipeline'].dropna().unique().tolist())
                        pipeline_filter = st.selectbox("🔍 Filter by Pipeline", pipelines, key="dl_pipeline_filter")
                with col2:
                    search = st.text_input("🔎 Search", placeholder="Search source/sink/type...", key="dl_search")
                ...
                if search:
                    ...
                    for c in filtered.columns:
                        mask |= filtered[c].astype(str).str.lower().str.contains(sl, na=False, regex=False)
                    filtered = filtered[mask]

                st.markdown(f"**{len(filtered):,} records**")
                self._safe_dataframe(filtered, height=420)

                csv_bytes = to_csv_bytes(filtered)
                st.download_button(
                    label="📥 Download Data Lineage CSV",
                    data=csv_bytes,
        ...
        # ──────────────────────────────────────────────────────────────────
        # SUB-TAB 2: Table Lineage (TableLineage sheet)
        # ──────────────────────────────────────────────────────────────────
        with sub_tab2:
            tl_df = safe_get_dataframe("TableLineage")

            if tl_df.empty:
                st.info("📊 No Table Lineage data available")
            else:
                # Metrics row
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total Rows", len(tl_df))
                with col2:
                    src_col = next((c for c in tl_df.columns if 'source' in c.lower() and 'table' in c.lower()), None)
                    st.metric("Unique Sources", tl_df[src_col].nunique() if src_col else 0)
                with col3:
                    tgt_col = next((c for c in tl_df.columns if 'target' in c.lower() and 'table' in c.lower()), None)
                    st.metric("Unique Targets", tl_df[tgt_col].nunique() if tgt_col else 0)
                with col4:
                    pl_col = next((c for c in tl_df.columns if 'pipeline' in c.lower()), None)
                    st.metric("Pipelines", tl_df[pl_col].nunique() if pl_col else 0)
                ...
                col1, col2, col3 = st.columns(3)
                with col1:
                    tl_pipeline_filter = "All"
                    if pl_col:
                        pl_opts = ["All"] + sorted(tl_df[pl_col].dropna().unique().tolist())
                        tl_pipeline_filter = st.selectbox("🔍 Filter by Pipeline", pl_opts, key="tl_pipeline_filter")
                with col2:
                    # SourceLinkedService filter
                    src_ls_col = next((c for c in tl_df.columns if 'source' in c.lower() and 'linked' in c.lower()), None)
                    tl_ls_filter = "All"
                    if src_ls_col:
                        ls_opts = ["All"] + sorted(tl_df[src_ls_col].dropna().unique().tolist())
                        tl_ls_filter = st.selectbox("🔗 Source Linked Service", ls_opts, key="tl_ls_filter")
                with col3:
                    tl_search = st.text_input("🔎 Search", placeholder="Search tables, paths...", key="tl_search")

                # Apply filters
                tl_filtered = tl_df.copy()
                if tl_pipeline_filter != "All" and pl_col:
                    tl_filtered = tl_filtered[tl_filtered[pl_col] == tl_pipeline_filter]
                if tl_ls_filter != "All" and src_ls_col:
                    tl_filtered = tl_filtered[tl_filtered[src_ls_col] == tl_ls_filter]
                if tl_search:
                    sl = tl_search.lower()
                    mask = pd.Series(False, index=tl_filtered.index)
                    for c in tl_filtered.columns:
                        mask |= tl_filtered[c].astype(str).str.lower().str.contains(sl, na=False, regex=False)
                    tl_filtered = tl_filtered[mask]

                st.markdown(f"**{len(tl_filtered):,} records**")
                self._safe_dataframe(tl_filtered, height=420)

                csv_bytes = to_csv_bytes(tl_filtered)
                st.download_button(
                    label="📥 Download Table Lineage CSV",
                    data=csv_bytes,
                    file_name="table_lineage.csv",
                    mime="text/csv",
                    key="tl_download"
                )
