...
class SQLParser:
    ...
    SQL_KEYWORDS = {
        ...
        # Column alias words that leak through FROM-regex on wide SELECT lists
        'ACCOUNT', 'PRODUCTION', 'WHICH', 'INDICATOR', 'MATERIAL', 'STATUS',
        'PLANT', 'CLIENT', 'NUMBER', 'DATE', 'TIME', 'TYPE', 'CODE', 'KEY',
        'LEVEL', 'QUANTITY', 'VALUE', 'BATCH', 'UNIT', 'METHOD', 'STOCK',
        'FIXED', 'MINIMUM', 'MAXIMUM', 'REORDER', 'SAFETY', 'GENERAL',
        'SPECIAL', 'PERIOD', 'SCRAP', 'LOT', 'SIZE', 'PROCUREMENT', 'STORAGE',
        'COSTS', 'ORDERING', 'DEPENDENT', 'REQUIREMENTS', 'INDIVIDUAL',
        'ASSEMBLY', 'DELIVERY', 'RECEIPT', 'PROCESSING', 'GOODS', 'PLANNING',
        'MANAGEMENT', 'MAINTENANCE', 'DELETION', 'VALUATION', 'CATEGORY',
        'COST', 'PRICE', 'SALES', 'BASE', 'REPORTING', 'TRADE', 'REVENUE',
        'NET', 'GROSS', 'FREE', 'ASSET', 'QUANTITY', 'SUMMARY',
    }
    ...
    def _strip_sql_comments(sql: str) -> str:
        """Remove SQL -- line comments and /* block comments */ before parsing.

        Also strips alias column expressions like:
            "Long Column Name" as "Alias_Name"
        which would otherwise pollute the FROM-clause regex when SQL is
        a wide SELECT statement with no explicit FROM table.
        """
        import re as _re
    ...
    @staticmethod
    def _normalize_hana_table(name: str) -> str:
        """Normalize SAP HANA _SYS_BIC virtual view references.

        HANA views live in the _SYS_BIC schema and use a slash-separated
        package path as their name:
            "_SYS_BIC"."nestle.dev.glb.ba.fico.COPA.models.calculation/CV_SALES_SUMMARY_PBI"
        After double-quote stripping this becomes:
            _SYS_BIC.nestle.dev.glb.ba.fico.COPA.models.calculation/CV_SALES_SUMMARY_PBI

        We convert to a readable canonical form:
            _SYS_BIC/CV_SALES_SUMMARY_PBI
        keeping only the schema prefix and the final view name after the last /.
        """
        if '_SYS_BIC' not in name.upper():
            return name
        import re as _re
        # Already normalized form: _SYS_BIC.package/ViewName
        slash_idx = name.rfind('/')
        if slash_idx != -1:
            view_name = name[slash_idx + 1:]
            return f'_SYS_BIC/{view_name}'
        return name
    ...
    def _extract_tables(sql_upper: str) -> 'Set[str]':
        ...
        # ── Strip comments FIRST to avoid alias words being parsed as tables ──
        sql_upper = SQLParser._strip_sql_comments(sql_upper).upper()
        ...
        # Pattern component: matches SQL Server brackets [schema].[table],
        # ANSI/Snowflake double-quotes "schema"."table", or plain identifiers.
        _tbl = r'(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+)(?:\.(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+))*'
        
        patterns = [
            # FROM clause
            rf'FROM\s+({_tbl})',
            # JOIN clauses
            rf'(?:INNER\s+|LEFT\s+(?:OUTER\s+)?|RIGHT\s+(?:OUTER\s+)?|FULL\s+(?:OUTER\s+)?|CROSS\s+)?JOIN\s+({_tbl})',
            # INTO clause
            rf'INTO\s+({_tbl})',
            # UPDATE clause
            rf'UPDATE\s+({_tbl})',
            # DELETE FROM
            rf'DELETE\s+FROM\s+({_tbl})',
            # TRUNCATE TABLE
            rf'TRUNCATE\s+TABLE\s+({_tbl})',
            # INSERT INTO
            rf'INSERT\s+INTO\s+({_tbl})',
        ]
        ...
        for pattern in patterns:
            try:
                ...
                for match in matches:
                    table = SQLParser._normalize_table_name(str(match).strip())
                    # Apply HANA _SYS_BIC normalization
                    table = SQLParser._normalize_hana_table(table)
                    if SQLParser._is_valid_table_name(table):
                        tables.add(table)
            except:
                pass

        # ── HANA _SYS_BIC view extraction (slash in view name breaks _tbl regex) ─
        # Pattern: FROM "_SYS_BIC"."package.path/VIEW_NAME"
        hana_pattern = r'FROM\s+"_SYS_BIC"\."([^"]+/([^"]+))"'
        for m in re.finditer(hana_pattern, sql_upper, re.IGNORECASE):
            view_name = m.group(2).strip()  # just the part after last /
            if view_name and len(view_name) > 2:
                tables.add(f'_SYS_BIC/{view_name}')
        ...
        for cte_name, cte_body in cte_bodies:
            ...
            for pattern in patterns:
                try:
                    ...
                    for match in matches:
                        # Handle tuple from multiple groups
                        raw = match if isinstance(match, str) else next((m for m in match if m), '')
                        table = SQLParser._normalize_table_name(str(raw).strip())
                        if SQLParser._is_valid_table_name(table):
    ...
    def _is_valid_table_name(name: str) -> bool:
        ...
        # Bare schema-only tokens that are never standalone table names
        SCHEMA_ONLY_TOKENS = {
            '_SYS_BIC', 'DBO', 'SYS', 'INFORMATION_SCHEMA', 'GUEST',
            'SAPBW', 'PUBLIC', 'TEMP', 'TEMPDB', 'MASTER', 'MODEL',
        }
        if name_upper in SCHEMA_ONLY_TOKENS:
            return False
        ...
        # Skip single-char or dummy table names
    ...
    def _normalize_table_name(name: str) -> str:
        """Strip SQL Server brackets and Snowflake/ANSI double-quotes from identifiers.

        [SC-SP].FCT_Segmentation       → SC-SP.FCT_Segmentation
        [dr].[PuritanSalesforceOrders] → dr.PuritanSalesforceOrders
        "EDW"."PRS"."S6_MDH_KLAH_V"   → EDW.PRS.S6_MDH_KLAH_V
        dbo.MyTable                     → dbo.MyTable  (unchanged)
        """
        if not name:
            return name
        result = re.sub(r'\[([^\]]*?)\]', r'\1', name)   # strip [brackets]
        result = re.sub(r'"([^"]*?)"', r'\1', result)      # strip "double quotes"
        return result
# ═══════════════════════════════════════════════════════════════════════════
...
class UltimateEnterpriseADFAnalyzer:
    ...
    def parse_pipeline(self, resource: dict):
        ...
        try:
            ...
            # Create pipeline record(s) — one row per parameter for exploded view
            params_dict = props.get('parameters', {})
            variables_str = self._format_dict(props.get('variables', {}))
            ...
            # Build base record (shared across all parameter rows)
            base_rec = {
                'Pipeline': name,
                'Folder': folder,
            ...
            if isinstance(params_dict, dict) and params_dict:
                # One row per parameter — with default value in a new column
                for param_name, param_def in params_dict.items():
                    ...
                    default_val = ''
                    if isinstance(param_def, dict):
                        raw_default = param_def.get('defaultValue', '')
                        default_val = TextSanitizer.sanitize_value(raw_default, 500)

                    row = dict(base_rec)  # copy base
                    row['Parameter'] = param_name
                    row['ParameterType'] = param_type
                    row['Parameters Default Value'] = default_val
                    self.results['pipelines'].append(row)
            else:
                # No parameters — single row with empty parameter columns
                row = dict(base_rec)
                row['Parameter'] = ''
                row['ParameterType'] = ''
                row['Parameters Default Value'] = ''
                self.results['pipelines'].append(row)
    ...
    def _extract_table_from_dataset_params(self, dataset_ref: dict) -> str:
        ...
        # Skip single-char dummy placeholders used with Generic datasets
        # (e.g. p_Schema='a', p_Table='a' when source comes from SQL query)
        dummy_values = {'A', 'S', 'B', 'X', '.', 'NA', 'N/A', 'DUMMY', 'NONE', 'NULL', 'TEST', ''}
        
        # Combine schema.table if both present
        if schema_name and table_name:
            # Skip dummy schemas AND dummy tables
            if schema_name.upper() in dummy_values or table_name.upper() in dummy_values:
                pass  # Fall through to ADLS path construction or return empty
            elif len(schema_name) <= 1 and len(table_name) <= 1:
                pass  # Single-char values like 'a.a', 's.s' are always placeholders
            else:
                return f"{schema_name}.{table_name}"
        
        if table_name and table_name.upper() not in dummy_values and len(table_name) > 1:
            return table_name
        
        # ── Fix 2: ADLS path construction from container/directory/file params ──
        container_params = ['p_container', 'container_name', 'Container', 'FileSys', 'p_ContainerName']
        directory_params = ['p_directory', 'FolderName', 'Folder', 'parquet_solution_path', 'p_FolderPath', 'DirectoryName']
        file_params = ['p_file', 'FileName', 'file_name', 'File', 'p_FileName']
        ...
        for pname in container_params:
            if pname in params:
                container = self._extract_value(params[pname])
                break
        for pname in directory_params:
            if pname in params:
                directory = self._extract_value(params[pname])
                break
        for pname in file_params:
            if pname in params:
                filename = self._extract_value(params[pname])
                break
        
        # ── Smart ADLS path builder ─────────────────────────────────────────
        # Resolves @concat parts, @variables(), @item(), @{...} interpolation
        # to produce human-readable paths like:
        #   OTR_OrderData_[datetime].parquet
        #   AS400/ASCP_RM/Archive/[Year]/[Month]
        #   inbound/NUSA/Segmentation/Archive/[datetime]
        import re as _re_p

        def _resolve_expr_part(expr_str):
            """
            Resolve a single ADF expression to a human-readable string.
            Returns the resolved string with semantic placeholders.
            """
            s = expr_str.strip() if expr_str else ''
            if not s or s in ('.', 'NA', 'DUMMY', 'null'):
                return None

            # Plain literal (no expression markers) — return as-is
            if not s.startswith('@') and '{' not in s and '$' not in s:
                return s

            # ── @concat(parts...) ──────────────────────────────────────────
            if _re_p.match(r'@[Cc]oncat\(', s):
                # Split concat args at top-level commas
                inner_m = _re_p.match(r'@[Cc]oncat\((.+)\)$', s, _re_p.DOTALL)
                if not inner_m:
                    # Fallback: extract first literal prefix
                    lit = _re_p.match(r"@[Cc]oncat\('([^']+)'", s)
                    return (lit.group(1) + '[dynamic]') if lit else '[dynamic]'

                inner = inner_m.group(1)
                # Parse top-level comma-separated args (respecting nested parens)
                args, depth, cur = [], 0, []
                for ch in inner:
                    if ch == '(': depth += 1
                    elif ch == ')': depth -= 1
                ...
                for arg in args:
                    ...
                    # variables('name') → [name]
                    elif _re_p.match(r"variables\('([^']+)'\)", arg):
                        vname = _re_p.match(r"variables\('([^']+)'\)", arg).group(1)
                        result_parts.append(f'[{vname}]')
                    # formatDateTime / formatDateTimeWithTimeZone → [datetime]
                    elif 'formatDateTime' in arg or 'formatDateTimeWithTimeZone' in arg:
                        result_parts.append('[datetime]')
                    # utcNow() / utcnow() → [datetime]
                    elif _re_p.match(r'utcNow\(\)|utcnow\(\)', arg, _re_p.IGNORECASE):
                        result_parts.append('[datetime]')
                    # convertFromUtc(...) → [datetime]
                    elif 'convertFromUtc' in arg:
                        result_parts.append('[datetime]')
                    # item().field → [field]
                    elif _re_p.match(r"item\(\)\.\w+", arg):
                        field = _re_p.match(r"item\(\)\.(\w+)", arg).group(1)
                        result_parts.append(f'[{field}]')
                    # pipeline().parameters.name → [@paramName]
                    elif 'pipeline().parameters.' in arg:
                        pm = _re_p.search(r'pipeline\(\)\.parameters\.(\w+)', arg)
                        result_parts.append(f'[@{pm.group(1) if pm else "param"}]')
                    # pipeline().globalParameters.name
                    elif 'pipeline().globalParameters.' in arg:
                        pm = _re_p.search(r'pipeline\(\)\.globalParameters\.(\w+)', arg)
                        result_parts.append(f'[@global:{pm.group(1) if pm else "param"}]')
                    # nested concat
                    elif _re_p.match(r'@?[Cc]oncat\(', arg):
                        sub = arg if arg.startswith('@') else '@' + arg
                        result_parts.append(_resolve_expr_part(sub) or '[dynamic]')
                    # @replace(expr, old, new) — extract base variable/item name
                    elif _re_p.match(r'@?replace\(', arg, _re_p.IGNORECASE):
                        # Try to extract variable name from first arg of replace()
                        vn = _re_p.search(r"variables\('([^']+)'\)", arg)
                        it = _re_p.search(r'item\(\)\.(\w+)', arg)
                        if vn:
                            result_parts.append(f'[{vn.group(1)}_modified]')
                        elif it:
                            result_parts.append(f'[{it.group(1)}_modified]')
                        else:
                            result_parts.append('[file_modified]')
                    # activity().output reference
                    elif 'activity(' in arg.lower():
                        result_parts.append('[activity_output]')
                    else:
                        result_parts.append('[dynamic]')

                # Join all parts, then strip redundant slashes that @concat embedded
                joined = ''.join(result_parts)
                # Remove double slashes introduced when literal args already contain trailing /
                import re as _re_slash
                joined = _re_slash.sub(r'/+', '/', joined)
                return joined.strip('/')


            # ── @variables('name') standalone ─────────────────────────────
            vm = _re_p.match(r"@variables\('([^']+)'\)$", s)
            if vm:
                return f'[{vm.group(1)}]'

            # ── @item().field standalone ───────────────────────────────────
            im = _re_p.match(r"@item\(\)\.(\w+)$", s)
            if im:
                return f'[{im.group(1)}]'

            # ── @pipeline().parameters.name ───────────────────────────────
            pm = _re_p.match(r"@pipeline\(\)\.parameters\.(\w+)$", s)
            if pm:
                return f'[@{pm.group(1)}]'

            # ── @replace(expr, old, new) standalone ───────────────────────
            if _re_p.match(r'@replace\(', s, _re_p.IGNORECASE):
                vn = _re_p.search(r"variables\('([^']+)'\)", s)
                it = _re_p.search(r'item\(\)\.(\w+)', s)
                ac = _re_p.search(r"activity\('([^']+)'\)", s)
                if vn:   return f'[{vn.group(1)}_modified]'
                elif it: return f'[{it.group(1)}_modified]'
                elif ac: return f'[{ac.group(1)}_output]'
                else:    return '[file_modified]'

            # ── activity().output reference ────────────────────────────────
            if _re_p.match(r'@activity\(', s, _re_p.IGNORECASE):
                ac = _re_p.search(r"activity\('([^']+)'\)", s)
                return f'[{ac.group(1)}_output]' if ac else '[activity_output]'

            # ── @{...} string interpolation ────────────────────────────────
            if '@{' in s:
                def replace_interp(m):
                    inner = m.group(1).strip()
                    if 'formatDateTime' in inner or 'utcNow' in inner.lower():
                        return '[datetime]'
                    elif inner.startswith('variables('):
                        vn = _re_p.search(r"variables\('([^']+)'\)", inner)
                        return f'[{vn.group(1) if vn else "var"}]'
                    elif inner.startswith('item().'):
                        return f'[{inner[7:]}]'
                    else:
                        return '[dynamic]'
                resolved = _re_p.sub(r'@\{([^}]+)\}', replace_interp, s)
                if resolved and resolved not in ('.', 'NA'):
                    return resolved

            # Fallback for any other expression
            return '[dynamic]'

        def _get_param_smart(keys):
            """Return resolved path segment for first matching key, with expr flag."""
            for k in keys:
                if k in params:
                    raw = params[k]
                    val = self._extract_value(raw)
                    is_expr = isinstance(raw, dict) and raw.get('type') == 'Expression'
                    return val, is_expr
            return '', False

        container_keys2 = ['p_container', 'container_name', 'Container', 'FileSys', 'p_ContainerName']
        dir_keys2       = ['p_directory', 'FolderName', 'Folder', 'parquet_solution_path', 'p_FolderPath', 'DirectoryName']
        file_keys2      = ['p_file', 'FileName', 'file_name', 'File', 'p_FileName']

        c_val, c_expr = _get_param_smart(container_keys2)
        d_val, d_expr = _get_param_smart(dir_keys2)
        f_val, f_expr = _get_param_smart(file_keys2)

        path_parts = []
        for part, is_expr in [(c_val, c_expr), (d_val, d_expr), (f_val, f_expr)]:
            if not part:
                continue
            if '@' in part or (is_expr and '$' in part):
                resolved = _resolve_expr_part(part)
                if resolved and resolved not in ('.', 'NA', 'DUMMY', 'null', None):
                    path_parts.append(resolved)
            elif is_expr:
                if part not in ('.', 'NA', 'DUMMY'):
                    path_parts.append(part)
            elif part not in ('.', 'NA', 'DUMMY'):
                path_parts.append(part)

        if path_parts:
            # Strip trailing slashes from each part (some @concat literals embed them)
            clean_parts = [p.strip('/') for p in path_parts if p and p.strip('/')]
            if clean_parts:
                result_path = '/'.join(clean_parts)
                # Normalize any double slashes that slipped through
                import re as _re_norm
                result_path = _re_norm.sub(r'/+', '/', result_path)
                return result_path
    ...
    def _parse_script_activity(self, parsed: ParsedActivity, type_props: dict):
        """Parse Script activity — extracts SQL + LinkedService + target/source tables."""
        # Extract LinkedService reference
        ls_ref = type_props.get('linkedService', {})
        ls_name = ''
        if isinstance(ls_ref, dict):
            ls_name = self._extract_name(ls_ref.get('referenceName', ''))
            if ls_name:
                parsed.role = f"Script via {ls_name}"
                parsed.values_info = f"LinkedService: {ls_name}"
                # Set both source and sink LS so TableLineage picks it up
                parsed.source_table = parsed.source_table  # keep existing
                # Store in integration_runtime field temporarily as LS carrier
                # Actually set via dataset field (re-used for LS name here)
                # We'll rely on ValuesInfo extraction in build_table_lineage

        # Extract SQL from all script blocks (may have multiple)
        ...
        if isinstance(scripts, list):
            for script_obj in scripts:
                if isinstance(script_obj, dict):
                    ...
                    if script_text:
                        all_sql_parts.append(self._extract_value(script_text))
        if all_sql_parts:
            combined_sql = '\n'.join(all_sql_parts)
            parsed.sql = combined_sql[:Config.MAX_SQL_LENGTH]

            # Pre-parse target/source tables from SQL for downstream lineage
            try:
                import re as _re_s
                _tbl_p = r'(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+)(?:\.(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+))*'
                tgt_tbls, src_tbls = [], []
                for pat, is_tgt in [
                    (rf'(?:INSERT\s+INTO|DELETE\s+FROM|TRUNCATE\s+TABLE|UPDATE)\s+({_tbl_p})', True),
                    (rf'FROM\s+({_tbl_p})', False),
                ]:
                    for m in _re_s.finditer(pat, combined_sql, _re_s.IGNORECASE):
                        raw = m.group(1).strip()
                        raw = _re_s.sub(r'\[([^\]]*?)\]', r'\1', raw)
                        raw = _re_s.sub(r'"([^"]*?)"', r'\1', raw)
                        if len(raw.replace('.','')) > 1 and raw.upper() not in {'SELECT', 'FROM', 'WHERE', 'AND'}:
                            (tgt_tbls if is_tgt else src_tbls).append(raw)
                if tgt_tbls:
                    parsed.sink_table = ', '.join(sorted(set(tgt_tbls)))
                    # Echo sink table into sink_sql so build_table_lineage can read it
                    parsed.sink_sql = f"-- targets: {parsed.sink_table}"
                if src_tbls:
                    parsed.source_table = ', '.join(sorted(set(src_tbls)))
                    parsed.source_sql = f"-- sources: {parsed.source_table}"
            except Exception:
                pass

        # Store LS in source/sink dataset field so it survives to_dict()
        # We use the dataset field as a LS carrier: "SCRIPT_LS:LS_NAME"
        if ls_name:
            parsed.dataset = f"SCRIPT_LS:{ls_name}"
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # TABLE LINEAGE BUILDER — Pipeline-level Source→Intermediate→Target
    # ═══════════════════════════════════════════════════════════════════════

    def build_table_lineage(self):
        """
        ✅ Build comprehensive Table-Level Lineage for every pipeline.

        For each pipeline, extracts:
          - TargetTable / TargetLinkedService
          - TransformationLayer (DataFlow / StoredProcedure / NONE)
          - IntermediateLayer (ADLS staging / staging table / NONE)
          - SourceTable / SourceLinkedService

        Uses SQLParser for FROM-clause extraction when queries exist.
        Handles: multi-hop, parallel copies, ForEach, dynamic params, SP.
        """
        self.logger.info("Building table lineage...")

        # ─── Pre-build lookup indexes ───────────────────────────────────
        # Dataset name → LinkedService name
        ds_to_ls = {}
        for ds in self.results.get('datasets', []):
            ds_name = ds.get('Dataset', '')
            ls_name = ds.get('LinkedService', '')
            if ds_name:
                ds_to_ls[ds_name] = ls_name

        # DataFlow name → sink tables (from dataflow_lineage)
        df_sinks = defaultdict(list)
        for dfl in self.results.get('dataflow_lineage', []):
            df_name = dfl.get('DataFlow', '')
            if df_name:
                df_sinks[df_name].append(dfl.get('SinkTable', ''))

        # ─── Group activities by pipeline (preserving execution order) ──
        pipeline_activities = defaultdict(list)
        for act in self.results.get('activities', []):
            pipeline_activities[act['Pipeline']].append(act)

        # ─── Process each pipeline ──────────────────────────────────────
        for pipeline_name, activities in pipeline_activities.items():
            # Sort by ExecutionStage then Sequence
            def _sort_key(a):
                stage = a.get('ExecutionStage', '') or 999
                seq = a.get('Sequence', 0) if isinstance(a.get('Sequence'), int) else 0
                return (stage if isinstance(stage, int) else 999, seq)

            sorted_acts = sorted(activities, key=_sort_key)

            # ─── STEP 0: Resolve pipeline variables ────────────────────────
            # Collect all SetVariable literal values so we can substitute
            # @{variables('VarName')} in dataset params, paths, and queries.
            pipeline_vars = {}  # {variable_name_lower: resolved_literal_value}
            for act in sorted_acts:
                if act.get('ActivityType') in ('SetVariable', 'AppendVariable'):
                    vi = act.get('ValuesInfo', '') or ''
                    # ValuesInfo format: "VarName = literal_value"
                    if '=' in vi:
                        parts = vi.split('=', 1)
                        vname = parts[0].strip().lower()
                        vval = parts[1].strip()
                        # Only store clean literal values (no expressions)
                        if '@' not in vval and len(vval) > 0:
                            pipeline_vars[vname] = vval

            def _apply_vars(s: str) -> str:
                """Substitute @{variables('Name')} and @variables('Name') with resolved values."""
                if not s or '@' not in s:
                    return s
                import re as _re
                def replacer(m):
                    vname = m.group(1).lower()
                    return pipeline_vars.get(vname, m.group(0))  # keep original if not found
                s = _re.sub(r"@\{?variables\('([^']+)'\)\}?", replacer, s, flags=_re.IGNORECASE)
                return s

            def _apply_bracket_vars(s: str) -> str:
                """Substitute [VarName] bracket placeholders (from smart path resolver)
                with known static SetVariable literal values.
                e.g. inbound/AmazonDataLake/[TableName]/[TableName].csv
                  →  inbound/AmazonDataLake/factInvoices/factInvoices.csv
                Only substitutes if the variable has a clean literal (no @ or /).
                Skips semantic markers: [datetime], [dynamic], [file_modified] etc.
                """
                if not s or '[' not in s:
                    return s
                import re as _re_bv
                # Markers that must NOT be substituted
                SKIP = {'datetime', 'dynamic', 'file_modified', 'file_modified', 'activity_output',
                        'ForEach', 'item', 'param'}
                def bv_replacer(m):
                    raw = m.group(1)           # e.g. "TableName" or "Year" or "datetime"
                    # Skip semantic markers and multi-word tokens
                    if raw.lower() in SKIP or raw.startswith('@') or raw.startswith('global:'):
                        return m.group(0)
                    # Skip _modified suffix markers
                    if raw.endswith('_modified') or raw.endswith('_output'):
                        return m.group(0)
                    val = pipeline_vars.get(raw.lower())
                    if val and '@' not in val and len(val) > 0:
                        return val
                    return m.group(0)           # keep [VarName] if not resolvable
                return _re_bv.sub(r'\[([^\]]+)\]', bv_replacer, s)

            def _extract_expr_hint(expr: str) -> str:
                """Extract static literal prefix from @concat('prefix', ...) dynamic expressions.

                @concat('NUSA/Segmentation/Archive/', formatDateTime(...)) → NUSA/Segmentation/Archive/[dynamic]
                @concat('V_MaterialGroupDescriptions_', convertFromUtc(...)) → V_MaterialGroupDescriptions_[dynamic]
                If the expression starts with a literal string, capture it + mark rest as [dynamic].
                """
                if not expr or '@' not in expr:
                    return expr
                import re as _re
                # Match @concat('literal', ...) or @Concat('literal', ...)
                m = _re.match(r"@[Cc]oncat\('([^']+)'", expr.strip())
                if m:
                    return m.group(1) + '[dynamic]'
                # Match @{concat('literal', ...)} or similar expression blocks
                m = _re.match(r"@\{[Cc]oncat\('([^']+)'", expr.strip())
                if m:
                    return m.group(1) + '[dynamic]'
                return expr
            ...
            # ─── STEP 0b: Back-patch Activities sheet with resolved values ─
            # The Activities records were populated during _parse_copy_activity
            # BEFORE pipeline_vars were known, so their SourceTable/SinkTable
            # may still contain @{variables('Name')} or [VarName] placeholders.
            # Now that we have pipeline_vars, resolve them in-place so the
            # Activities sheet (and downstream TableLineage) shows literal values
            # like "dl.factInvoices" or "inbound/AmazonDataLake/factPayments/..."
            import re as _re_bp
            for act in sorted_acts:
                for field in ('SourceTable', 'SinkTable'):
                    raw = act.get(field, '') or ''
                    if raw and ('@' in raw or '[' in raw):
                        resolved = _apply_vars(raw)
                        resolved = _apply_bracket_vars(resolved)
                        if resolved != raw:
                            act[field] = resolved

                # Extra pass: if SourceTable is still a @dataset: placeholder
                # but the activity has a real sqlReaderQuery, extract from SQL.
                # This covers pipelines where the dataset has dummy params (e.g.
                # p_Schema='a', p_Table='a') but the actual source is in the SQL.
                src_raw = act.get('SourceTable', '') or ''
                src_sql = act.get('SourceSQL', '') or ''
                if '@dataset:' in src_raw and src_sql.strip():
                    sql_resolved = _apply_vars(src_sql)
                    try:
                        extracted = SQLParser._extract_tables(sql_resolved.upper())
                        clean = [t for t in extracted if '@' not in t and '$' not in t]
                        if clean:
                            act['SourceTable'] = ', '.join(sorted(clean))
                    except Exception:
                        pass
            ...
            for act in sorted_acts:
                ...
                if act_type == 'Copy':
                    copy_acts.append(act)
                elif act_type == 'ExecuteDataFlow':
                    dataflow_acts.append(act)
                elif act_type in ('SqlServerStoredProcedure', 'StoredProcedure'):
                    sp_acts.append(act)
                elif act_type == 'Script':
                    script_acts.append(act)
                elif act_type in ('SetVariable', 'AppendVariable'):
                    set_variable_acts.append(act)

            # ─── Extract intermediate layer from SetVariable ────────────
            intermediate_layer = 'NONE'
            for sv in set_variable_acts:
                values_info = sv.get('ValuesInfo', '') or ''
                if values_info and '/' in values_info:
                    path_parts = []
                    for part in str(values_info).split(','):
                        part = part.strip()
                        if part and '/' in part:
                            # Apply variable substitution on the path part
                            path_parts.append(_apply_vars(part))
                    if path_parts:
                        intermediate_layer = ' | '.join(path_parts)
                        break
                # Also detect variable-set path value (e.g. inbound_path)
                if '=' in values_info:
                    vval = values_info.split('=', 1)[1].strip()
                    if '/' in vval:
                        intermediate_layer = f'inbound_path = {vval[:120]}'
                        break

            # ─── No data-producing activities → Script or flag ──────────
            if not copy_acts and not dataflow_acts:
                if script_acts:
                    # _parse_script_activity pre-populates SinkTable/SourceTable
                    # from INSERT INTO/DELETE FROM/FROM clauses, and stores
                    # LS in ValuesInfo as "LinkedService: LS_ASA"
                    for sa in script_acts:
                        sa_tgt = sa.get('SinkTable', '') or ''
                        sa_src = sa.get('SourceTable', '') or ''
                        vi = sa.get('ValuesInfo', '') or ''
                        sa_ls = vi.replace('LinkedService:', '').strip() if vi.startswith('LinkedService:') else ''
                        if not sa_ls:
                            # Fallback: read from Dataset field carrier "SCRIPT_LS:LS_NAME"
                            ds_field = sa.get('Dataset', '') or ''
                            if ds_field.startswith('SCRIPT_LS:'):
                                sa_ls = ds_field.replace('SCRIPT_LS:', '').strip()
                        if not sa_ls:
                            sa_ls = sa.get('SinkLinkedService', '') or sa.get('SourceLinkedService', '') or ''
                        self.results['table_lineage'].append({
                            'TargetTable': sa_tgt if sa_tgt else 'Script — Manual Review Required',
                            'TargetLinkedService': sa_ls,
                            'TransformationLayer': sa.get('Activity', 'Script'),
                            'IntermediateLayer': intermediate_layer,
                            'SourceTable': sa_src if sa_src else 'Script — Manual Review Required',
                            'SourceLinkedService': sa_ls,
                            'PipelineName': pipeline_name
                        })
                elif sp_acts:
                    self.results['table_lineage'].append({
                        'TargetTable': 'Stored Procedure — Manual Review Required',
                        'TargetLinkedService': '',
                        'TransformationLayer': ', '.join(a.get('StoredProcedure', a.get('Activity', '')) for a in sp_acts),
                        'IntermediateLayer': intermediate_layer,
                        'SourceTable': 'Stored Procedure — Manual Review Required',
                        'SourceLinkedService': '',
                        'PipelineName': pipeline_name
                    })
                else:
                    self.results['table_lineage'].append({
                        'TargetTable': 'Non-Copy Pipeline — No direct table lineage extractable',
                        'TargetLinkedService': '',
                        'TransformationLayer': 'NONE',
            ...
            # ─── Process Copy Activities ────────────────────────────────
            copy_rows = []
            for idx, act in enumerate(copy_acts):
                source_table = act.get('SourceTable', '') or ''
                sink_table = act.get('SinkTable', '') or ''
                source_sql = act.get('SourceSQL', '') or ''
                dataset_field = act.get('Dataset', '') or ''
                act_name = act.get('Activity', '')

                # ── Pre-resolve source/sink from dataset params FIRST ───
                # Apply variable substitution before SQL extraction so that
                # values like "dl.@{variables('TableName')}" → "dl.factInvoices"
                # are resolved BEFORE the SQL block decides to override them.
                source_table = _apply_vars(source_table)
                sink_table   = _apply_vars(sink_table)
                source_table = _apply_bracket_vars(source_table)
                sink_table   = _apply_bracket_vars(sink_table)

                # ── Scenario 1: Query FROM-clause extraction ────────────
                # Only use SQL extraction if source_table is STILL empty or
                # unresolved (contains '@' expressions) after the above pass.
                # This prevents the uppercased SQL parser output (e.g.
                # "DL.FACTINVOICES") from clobbering an already-resolved
                # dataset-param value like "dl.factInvoices".
                # Treat @dataset: placeholder tokens as unresolved expressions:
                # they appear when _extract_table_from_dataset_params returned empty
                # (e.g. dummy values like p_Schema='a', p_Table='a') and the fallback
                # field builder wrote "@dataset:p_Schema.@dataset:p_Table".  If the
                # activity also has a sqlReaderQuery we should extract the real source
                # table from that instead of labelling it ForEach-Parameterized.
                source_has_expr = ('@{' in source_table
                                   or '@pipeline' in source_table.lower()
                                   or '@dataset:' in source_table)
                if source_sql and source_sql.strip() and (not source_table or source_has_expr):
                    # Apply variable substitution on the SQL before parsing
                    source_sql_resolved = _apply_vars(source_sql)
                    sql_upper = source_sql_resolved.upper()
                    extracted_tables = []
                    try:
                        extracted_tables = SQLParser._extract_tables(sql_upper)
                    except Exception:
                        pass

                    if extracted_tables:
                        # Clean out parameter tokens if they got parsed as tables
                        clean_tables = [t for t in extracted_tables if '@' not in t and '$' not in t]
                        if clean_tables:
                            source_table = ', '.join(sorted(clean_tables))
                        else:
                            source_table = f"@{{parameter}} — Dynamic — Resolve at Runtime"
                    elif '@{' in source_sql or '@pipeline' in source_sql.lower():
                        source_table = f"@{{parameter}} — Dynamic — Resolve at Runtime"


                # ── Apply variable resolution to ADLS path in sink_table ─
                # Also extract @concat() literal prefixes for dynamic paths
                if sink_table and '@' in sink_table:
                    resolved = _apply_vars(sink_table)
                    if '@' in resolved:  # still dynamic after var substitution
                        hint = _extract_expr_hint(resolved)
                        if hint and hint != resolved:
                            sink_table = hint
                        else:
                            sink_table = resolved  # keep resolved form
                    else:
                        sink_table = resolved

                # ── Resolve LinkedService from Dataset ──────────────────
                source_ls = ''
                sink_ls = ''
                if dataset_field:
                    # Dataset field format: "IN:source_ds | OUT:sink_ds"
                    if 'IN:' in dataset_field and 'OUT:' in dataset_field:
                        parts = dataset_field.split(' | ')
                        src_ds = next((p.replace('IN:', '').strip() for p in parts if 'IN:' in p), '')
                        snk_ds = next((p.replace('OUT:', '').strip() for p in parts if 'OUT:' in p), '')
                        source_ls = ds_to_ls.get(src_ds, act.get('SourceLinkedService', '') or '')
                        sink_ls = ds_to_ls.get(snk_ds, act.get('SinkLinkedService', '') or '')
                    else:
                        source_ls = ds_to_ls.get(dataset_field, '')

                # Fallback to activity-level LS fields
                if not source_ls:
                    source_ls = act.get('SourceLinkedService', '') or ''
                if not sink_ls:
                    sink_ls = act.get('SinkLinkedService', '') or ''

                # ── Clean up @dataset: parameter tokens ────────────────
                # @dataset:p_table means table name is set per-iteration (ForEach)
                # @dataset:RelativeURL means REST API endpoint parameter
                # IMPORTANT: only apply this label when no SQL resolved the source.
                # If SQL extraction already set source_table to a real table name
                # (e.g. SC-SP.FCT_SEGMENTATION) we must NOT overwrite it here.
                import re as _re_ds
                if source_table and '@dataset:' in source_table:
                    # Extract the param name(s) for a human-readable label
                    param_names = _re_ds.findall(r'@dataset:(\w+)', source_table)
                    if param_names:
                        src_ls_hint = source_ls or act.get('SourceLinkedService', '') or ''
                        param_label = ', '.join(sorted(set(param_names)))
                        source_table = f'ForEach-Parameterized ({param_label}) — set at runtime [{src_ls_hint}]'
                    else:
                        source_table = 'ForEach-Parameterized — set at runtime'
                if sink_table and '@dataset:' in sink_table:
                    param_names = _re_ds.findall(r'@dataset:(\w+)', sink_table)
                    if param_names:
                        snk_ls_hint = sink_ls or act.get('SinkLinkedService', '') or ''
                        param_label = ', '.join(sorted(set(param_names)))
                        sink_table = f'ForEach-Parameterized ({param_label}) — set at runtime [{snk_ls_hint}]'
                    else:
                        sink_table = 'ForEach-Parameterized — set at runtime'

                # ── Flag remaining dynamic table names ──────────────────
                if source_table and ('@{' in source_table or '@pipeline' in source_table.lower()):
                    source_table = f"{source_table} — Dynamic — Resolve at Runtime"
                if sink_table and ('@{' in sink_table or '@pipeline' in sink_table.lower()):
                    sink_table = f"{sink_table} — Dynamic — Resolve at Runtime"
            ...
            # If Copy N's sink matches Copy N+1's source → intermediate
            if len(copy_rows) >= 2 and intermediate_layer == 'NONE':
                for i in range(len(copy_rows) - 1):
                    curr_sink = copy_rows[i]['sink_table'].lower().strip()
                    next_src = copy_rows[i + 1]['source_table'].lower().strip()
                    if curr_sink and next_src and (
                        curr_sink == next_src or
                        curr_sink in next_src or
                        next_src in curr_sink
                    ):
                        intermediate_layer = copy_rows[i]['sink_table']

            # ─── Fix 3: Cross-Activity Chain Resolution ─────────────────
            # Backfill UNKNOWN source/target from sibling activities
            if len(copy_rows) >= 2:
                # Find first known source across all activities
                first_known_source = ''
                first_known_source_ls = ''
                for cr in copy_rows:
                    if cr['source_table'] and cr['source_table'] not in ('UNKNOWN', ''):
                        first_known_source = cr['source_table']
                        first_known_source_ls = cr['source_ls']
                        break
                
                # Find last known target across all activities
                last_known_target = ''
                last_known_target_ls = ''
                for cr in reversed(copy_rows):
                    if cr['sink_table'] and cr['sink_table'] not in ('UNKNOWN', ''):
                        last_known_target = cr['sink_table']
                        last_known_target_ls = cr['sink_ls']
                        break
                
                # If target is still UNKNOWN but pipeline has a DataFlow, pull target from DataFlow lineage
                if not last_known_target and dataflow_acts:
                    for dfa in reversed(dataflow_acts):
                        df_name = dfa.get('DataFlow', '')
                        for dfl in self.results.get('dataflow_lineage', []):
                            if dfl.get('DataFlow') == df_name and dfl.get('SinkTable'):
                                last_known_target = dfl.get('SinkTable')
                                last_known_target_ls = dfl.get('SinkLinkedService') or ds_to_ls.get(dfl.get('SinkDataset', ''), '')
                                # Ensure we don't grab something unparsed
                                if last_known_target and 'UNKNOWN' not in last_known_target:
                                    break
                        if last_known_target:
                            break
                
                # Detect if this is a chain (source → ADLS → target)
                has_unknown_source = any(not cr['source_table'] or cr['source_table'] == 'UNKNOWN' for cr in copy_rows)
                has_unknown_target = any(not cr['sink_table'] or cr['sink_table'] == 'UNKNOWN' for cr in copy_rows)
                is_chain = first_known_source and last_known_target and (has_unknown_source or has_unknown_target)
                
                if is_chain:
                    # Find the intermediate ADLS hop
                    for cr in copy_rows:
                        src = cr['source_table'] or 'UNKNOWN'
                        tgt = cr['sink_table'] or 'UNKNOWN'
                        # Activity that writes to an intermediate (known source, unknown or ADLS target)
                        if src != 'UNKNOWN' and (tgt == 'UNKNOWN' or '/' in tgt) and intermediate_layer == 'NONE':
                            intermediate_layer = tgt if tgt != 'UNKNOWN' else 'ADLS staging (parameterized)'

            # ─── Determine transformation layer ────────────────────────
            transformation_layer = 'NONE'
            if dataflow_acts:
                df_names = [a.get('DataFlow', '') for a in dataflow_acts if a.get('DataFlow')]
                transformation_layer = ', '.join(df_names) if df_names else 'NONE'
            elif sp_acts:
                sp_names = [a.get('StoredProcedure', '') or a.get('Activity', '') for a in sp_acts]
                transformation_layer = ', '.join(sp_names)

            # ─── Build output rows ──────────────────────────────────────
            # Make sure chain variables are available if there was only 1 row
            if 'is_chain' not in locals():
                is_chain = False

            if copy_rows:
                for cr in copy_rows:
                    final_source = cr['source_table'] or 'UNKNOWN'
                    final_target = cr['sink_table'] or 'UNKNOWN'
                    final_src_ls = cr['source_ls']
                    final_tgt_ls = cr['sink_ls']

                    # Apply chain logic to backfill UNKNOWNs
                    if is_chain:
                        if final_source == 'UNKNOWN' and first_known_source:
                            final_source = first_known_source
                            final_src_ls = first_known_source_ls
                        if final_target == 'UNKNOWN' and last_known_target:
                            final_target = last_known_target
                            final_tgt_ls = last_known_target_ls

                    self.results['table_lineage'].append({
                        'TargetTable': final_target,
                        'TargetLinkedService': final_tgt_ls,
                        'TransformationLayer': transformation_layer,
                        'IntermediateLayer': intermediate_layer,
                        'SourceTable': final_source,
                        'SourceLinkedService': final_src_ls,
                        'PipelineName': pipeline_name
                    })

                # ── Fix 4: DataFlow inline datasetParameters → extra rows ─
                # Each sink param group in the ExecuteDataFlow becomes a separate
                # lineage row.  Source = the ADLS files that Copy activities wrote.
                # Execution order: Copy activities → DataFlow → Script.
                if dataflow_acts:
                    # Collect the file sinks produced by Copy activities (the feed into the DataFlow)
                    copy_file_sources = sorted(set(
                        cr['sink_table'] for cr in copy_rows
                        if cr['sink_table'] and cr['sink_table'] not in ('UNKNOWN', '')
                    ))
                    copy_file_source_str = ', '.join(copy_file_sources) if copy_file_sources else 'UNKNOWN'
                    copy_src_ls = next((cr['sink_ls'] for cr in copy_rows if cr['sink_ls']), '')

                    for dfa in dataflow_acts:
                        df_name = dfa.get('DataFlow', '')
                        df_sink_table = dfa.get('SinkTable', '') or ''
                        df_sink_ls   = dfa.get('SinkLinkedService', '') or ds_to_ls.get('', '')

                        # Try resolving LS from DataFlow lineage
                        if not df_sink_ls:
                            for dfl in self.results.get('dataflow_lineage', []):
                                if dfl.get('DataFlow') == df_name and dfl.get('SinkTable') == df_sink_table:
                                    df_sink_ls = dfl.get('SinkLinkedService') or ds_to_ls.get(dfl.get('SinkDataset', ''), '')
                                    break
                            if not df_sink_ls:
                                for dfl in self.results.get('dataflow_lineage', []):
                                    if dfl.get('DataFlow') == df_name:
                                        df_sink_ls = dfl.get('SinkLinkedService') or ds_to_ls.get(dfl.get('SinkDataset', ''), '')
                                        break

                        if df_sink_table and df_sink_table not in ('UNKNOWN', ''):
                            self.results['table_lineage'].append({
                                'TargetTable': df_sink_table,
                                'TargetLinkedService': df_sink_ls,
                                'TransformationLayer': df_name,
                                'IntermediateLayer': copy_file_source_str,
                                'SourceTable': copy_file_source_str,
                                'SourceLinkedService': copy_src_ls,
                                'PipelineName': pipeline_name
                            })
                        else:
                            # Fall back to dataflow_lineage sheet entries
                            for dfl in self.results.get('dataflow_lineage', []):
                                if dfl.get('DataFlow') == df_name and dfl.get('SinkTable'):
                                    snk_ds = dfl.get('SinkDataset', '')
                                    self.results['table_lineage'].append({
                                        'TargetTable': dfl['SinkTable'],
                                        'TargetLinkedService': dfl.get('SinkLinkedService') or ds_to_ls.get(snk_ds, ''),
                                        'TransformationLayer': df_name,
                                        'IntermediateLayer': copy_file_source_str,
                                        'SourceTable': copy_file_source_str,
                                        'SourceLinkedService': copy_src_ls,
                ...
                # ── Script activities after Copy/DataFlow → target rows ───
                for sa in script_acts:
                    sa_sql = sa.get('SourceSQL', '') or ''
                    sa_ls = sa.get('SinkLinkedService', '') or sa.get('SourceLinkedService', '') or ''
                    sc_tgt = set()
                    sc_src = set()
                    if sa_sql:
                        try:
                            import re as _re3
                            _tbl_p2 = r'(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+)(?:\.(?:\[[\w\s\-]+\]|"[\w\s\-]+"|\w+))*'
                            for pat2, is_tgt2 in [
                                (rf'(?:INSERT\s+INTO|DELETE\s+FROM|TRUNCATE\s+TABLE|UPDATE)\s+({_tbl_p2})', True),
                                (rf'FROM\s+({_tbl_p2})', False),
                            ]:
                                for m2 in _re3.finditer(pat2, sa_sql, _re3.IGNORECASE):
                                    t2 = SQLParser._normalize_table_name(m2.group(1).strip())
                                    if SQLParser._is_valid_table_name(t2):
                                        (sc_tgt if is_tgt2 else sc_src).add(t2)
                        except Exception:
                            pass
                    if sc_tgt:
                        self.results['table_lineage'].append({
                            'TargetTable': ', '.join(sorted(sc_tgt)),
                            'TargetLinkedService': sa_ls,
                            'TransformationLayer': sa.get('Activity', 'Script'),
                            'IntermediateLayer': intermediate_layer,
                            'SourceTable': ', '.join(sorted(sc_src)) if sc_src else 'Script — see SQL',
                            'SourceLinkedService': sa_ls,
                            'PipelineName': pipeline_name
                        })
            elif dataflow_acts:
                # DataFlow-only pipeline (no Copy activities)
                for dfa in dataflow_acts:
                    df_name = dfa.get('DataFlow', '')
                    # Cross-reference dataflow_lineage for source/sink tables
                    for dfl in self.results.get('dataflow_lineage', []):
                        if dfl.get('DataFlow') == df_name:
                            src_ds = dfl.get('SourceDataset', '')
                            snk_ds = dfl.get('SinkDataset', '')
                            self.results['table_lineage'].append({
                                'TargetTable': dfl.get('SinkTable', '') or 'UNKNOWN',
                                'TargetLinkedService': ds_to_ls.get(snk_ds, ''),
                                'TransformationLayer': df_name,
                                'IntermediateLayer': intermediate_layer,
                                'SourceTable': dfl.get('SourceTable', '') or 'UNKNOWN',
                                'SourceLinkedService': ds_to_ls.get(src_ds, ''),
                                'PipelineName': pipeline_name
                            })

        # Remove identical duplicate rows generated from backfilled chains
        # AND aggregate parallel sources into a single row per unique source-to-target path (Prompt Scenario 2 & 5)
        path_groups = {}
        for row in self.results.get('table_lineage', []):
            group_key = (
                row['PipelineName'],
                row['TargetTable'],
                row['TargetLinkedService'],
                row['IntermediateLayer'],
                row['TransformationLayer']
            )
            if group_key not in path_groups:
                path_groups[group_key] = row
            else:
                # Combine distinct SourceTables
                current_sources = path_groups[group_key]['SourceTable']
                new_source = row['SourceTable']
                if current_sources != new_source and current_sources != 'UNKNOWN':
                    if new_source != 'UNKNOWN':
                        sources_list = [s.strip() for s in current_sources.split(',')]
                        new_list = [s.strip() for s in new_source.split(',')]
                        # Unique list preserving order
                        combined = sorted(set(sources_list + new_list))
                        path_groups[group_key]['SourceTable'] = ', '.join(combined)
                elif current_sources == 'UNKNOWN' and new_source != 'UNKNOWN':
                    path_groups[group_key]['SourceTable'] = new_source
                    
                # Combine SourceLinkedService if different
                current_sls = path_groups[group_key]['SourceLinkedService']
                new_sls = row['SourceLinkedService']
                if new_sls and new_sls != current_sls:
                    if current_sls:
                        sls_list = [s.strip() for s in current_sls.split(',')]
                        if new_sls not in sls_list:
                            path_groups[group_key]['SourceLinkedService'] = current_sls + ', ' + new_sls
                    else:
                        path_groups[group_key]['SourceLinkedService'] = new_sls

        self.results['table_lineage'] = list(path_groups.values())

        self.logger.info(f"Table lineage: {len(self.results['table_lineage'])} unique paths across {len(pipeline_activities)} pipelines")
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════════
            # Step 10b: Build Table-Level Lineage
            # ═══════════════════════════════════════════════════════════════
            self.build_table_lineage()
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    def _write_summary_sheet(self, writer, timestamp: str):
        ...
        summary_data = [
            ...
            {'Category': 'PARSED DATA', 'Metric': 'Table Lineage Records', 'Value': len(self.results['table_lineage']), 'Details': f" See sheet: TableLineage"},
    ...
    def _placeholder_df(self, sheet_name: str) -> pd.DataFrame:
        schema_map = {
            ...
            'TableLineage': ['TargetTable', 'TargetLinkedService', 'TransformationLayer', 'IntermediateLayer', 'SourceTable', 'SourceLinkedService', 'PipelineName'],
    ...
    def _write_analysis_sheets(self, writer):
        ...
        analysis_sheets = [
            ...
            ('TableLineage', self.results['table_lineage']),
    ...
    def _write_data_dictionary_sheet(self, writer):
        ...
        dictionary_data = [
            ...
            # TABLE LINEAGE
            # ═══════════════════════════════════════════════════════════════
            {'Sheet': 'TableLineage', 'Column': 'TargetTable', 'Description': 'Final destination table where data lands at the end of the pipeline', 'DataType': 'Text', 'Example': 'dw.FactSales'},
            {'Sheet': 'TableLineage', 'Column': 'TargetLinkedService', 'Description': 'Linked service connection for the target table', 'DataType': 'Text', 'Example': 'LS_SYNAPSE'},
            {'Sheet': 'TableLineage', 'Column': 'TransformationLayer', 'Description': 'DataFlow or Stored Procedure name applied (NONE if direct copy)', 'DataType': 'Text', 'Example': 'df_TransformSales'},
            {'Sheet': 'TableLineage', 'Column': 'IntermediateLayer', 'Description': 'Staging table or ADLS path used between source and target (NONE if direct)', 'DataType': 'Text', 'Example': 'ADLS:/raw/sales/'},
            {'Sheet': 'TableLineage', 'Column': 'SourceTable', 'Description': 'Originating table — extracted from SQL FROM clause when available', 'DataType': 'Text', 'Example': 'dbo.SalesTransactions'},
            {'Sheet': 'TableLineage', 'Column': 'SourceLinkedService', 'Description': 'Linked service connection for the source table', 'DataType': 'Text', 'Example': 'LS_SQLSERVER'},
            {'Sheet': 'TableLineage', 'Column': 'PipelineName', 'Description': 'Pipeline that performs this data movement', 'DataType': 'Text', 'Example': 'pl_FullLoad_Sales'},
