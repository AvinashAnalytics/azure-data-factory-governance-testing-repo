...
def render_chatgpt_chat_tab(excel_data: Dict[str, pd.DataFrame] = None):
    ...
    if all_pipeline_names:
        with st.expander(f"🔍 **Pipeline Filter** (optional) — {len(all_pipeline_names)} available", expanded=False):
            ...
            # Use widget key as single source of truth (fixes hosted deployment)
            ...
            st.multiselect(
                ...
                on_change=_on_gpt_filter_change,
            )
            # Always sync after render
            st.session_state.gpt_pipeline_filter = st.session_state.get("gpt_pipeline_multiselect", [])

            if st.session_state.gpt_pipeline_filter:
                st.success(f"🎯 {len(st.session_state.gpt_pipeline_filter)} pipeline(s) selected — context will be filtered.")
