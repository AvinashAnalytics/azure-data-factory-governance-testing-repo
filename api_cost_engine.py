...
═══════════════════════════════════════════════════════════════════════════
Tracks actual API usage across Gemini & OpenAI, calculates costs with
current per-token pricing, generates comparison reports & Excel exports.
...
ALL_PRICING: Dict[str, ModelPricing] = {**GEMINI_PRICING, **OPENAI_PRICING}
...
def render_cost_analysis_tab(excel_data: Optional[Dict[str, pd.DataFrame]] = None):
    ...
    with tab1:
        if summary["total_calls"] == 0:
            st.info("🔍 No API calls recorded yet. Use the **🤖 AI Chat** or **🧠 ChatGPT** tabs to start analyzing, and your usage will appear here automatically.")
        else:
            # Provider breakdown
            st.subheader("Provider Breakdown")
            pcols = st.columns(len(summary["by_provider"]))
            provider_colors = {"Google": "#34d399", "OpenAI": "#818cf8"}
            ...
            st.dataframe(pd.DataFrame(history_rows), use_container_width=True, hide_index=True, height=400)
    ...
    with tab2:
        ...
        with fcol1:
            provider_filter = st.multiselect("Filter by Provider", ["Google", "OpenAI"], default=["Google", "OpenAI"], key="cost_provider_filter")
        with fcol2:
    ...
    with tab3:
        ...
        st.dataframe(pd.DataFrame(gemini_rows), use_container_width=True, hide_index=True)
        ...
        st.dataframe(pd.DataFrame(openai_rows), use_container_width=True, hide_index=True)

        st.markdown("---")
        st.caption("💡 **Tip:** Google Gemini models have a free tier — great for testing and development. OpenAI models require a paid API key. Prices updated March 2026.")
    ...
    with tab4:
        ...
        if st.button("📊 Generate Cost Report", key="gen_cost_report", use_container_width=True, type="primary"):
