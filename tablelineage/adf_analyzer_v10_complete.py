...
class SQLParser:
    ...
    def _extract_tables(sql_upper: str) -> Set[str]:
        ...
        # Pattern component: matches both regular identifiers and bracket-quoted identifiers
        # e.g. dbo.Table, [SC-SP].FCT_Table, [dr].[PuritanSalesforceOrders]
    ...
    def _normalize_table_name(name: str) -> str:
        """Strip SQL Server bracket-quoting from table identifiers.
...
class UltimateEnterpriseADFAnalyzer:
    ...
    def _extract_table_from_dataset_params(self, dataset_ref: dict) -> str:
        ...
        # Build ADLS path from components (skip expression values)
        path_parts = []
        for part in [container, directory, filename]:
            if part and '@' not in part and '$' not in part and part not in ('.', 'NA', 'DUMMY'):
    ...
    def build_table_lineage(self):
        ...
        for pipeline_name, activities in pipeline_activities.items():
            ...
            for sv in set_variable_acts:
                values_info = sv.get('ValuesInfo', '')
                if values_info:
                    # Look for ADLS-like paths in ValuesInfo
                    # Pattern: SetContainer + SetPath → "Inbound/GlobalSC/..."
                    path_parts = []
            ...
            # ─── No data-producing activities → skip or flag ────────────
